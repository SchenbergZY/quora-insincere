...
def runtraining(model_type = 31,save_name = '',lr = 0.1,batch_size = 1000,nb_epoch = 5,is_preprocessing = True,best_threshold = None,is_GPU = False，finetune=True):
    ...
    def num_FE(temp_df):
        df = pd.DataFrame()
        df['total_length'] = temp_df['question_text'].apply(len)
        df['capitals'] = temp_df['question_text'].apply(lambda comment: sum(1 for c in comment if c.isupper()))
        df['caps_vs_length'] = df.apply(lambda row: float(row['capitals'])/float(row['total_length']),
                                        axis=1)
        df['num_exclamation_marks'] = temp_df['question_text'].apply(lambda comment: comment.count('!'))
        df['num_question_marks'] = temp_df['question_text'].apply(lambda comment: comment.count('?'))
        df['num_punctuation'] = temp_df['question_text'].apply(
            lambda comment: sum(comment.count(w) for w in '.,;:'))
        df['num_symbols'] = temp_df['question_text'].apply(
            lambda comment: sum(comment.count(w) for w in '*#$'))
        df['num_words'] = temp_df['question_text'].apply(lambda comment: len(comment.split()))
        df['num_unique_words'] = temp_df['question_text'].apply(
            lambda comment: len(set(w for w in comment.split())))
        df['words_vs_unique'] = df['num_unique_words'] / df['num_words']
        return df.values.astype(np.float32)
    num_train_X = num_FE(train_df)
    num_test_X = num_FE(test_df)
    '''
    print('num_embedding build finish')
